...
# architecture-matched executable using PyInstaller, and copy the result
# into the macOS app's bundle Resources so BackendProcessManager can find it.
#
# Run this on the target Mac architecture with an active Python venv that has the backend
...
if ! "$PYTHON" -c "import faster_whisper" 2>/dev/null; then
    echo "❌ faster-whisper is required for a distributable RepoWhisper build."
    exit 1
fi
FASTER_WHISPER_FLAGS=(--collect-all faster_whisper --hidden-import ctranslate2)
...
# Make sure the executable matches the host architecture used for this build.
ARCH_INFO=$(file "$BUILT_BINARY")
if ! echo "$ARCH_INFO" | grep -q "$TARGET_ARCH"; then
    echo "❌ Built binary is not $TARGET_ARCH. file output:"
    echo "$ARCH_INFO"
    exit 1
fi
...
Next steps:
  1. Product → Archive → Distribute App → Developer ID → upload for notarization.
  2. After Xcode exports the .app, run:
